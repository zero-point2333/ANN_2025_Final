...
def get_shape(val: object) -> typing.List[int]:
    ...
    if hasattr(val, 'type'):
        # Jittor的tensor类型处理
        if hasattr(val.type(), 'dtype'):
            shape = val.shape
            if not shape:
                return [1]
            return list(shape)
    elif isinstance(val, jt.Var):
        shape = val.shape
        if not shape:
            return [1]
        return list(shape)
...
def einsum_flop_jit(
    ...
    assert len(inputs) == 2
    equation = str(inputs[0])  # Jittor中获取方程字符串
...
def norm_flop_counter(affine_arg_index: int) -> Handle:
    ...
    def norm_flop_jit(inputs: List[Any], outputs: List[Any]) -> Number:
        ...
        # Inputs[0] contains the shape of the input.
        input_shape = get_shape(inputs[0])
        has_affine = len(inputs) > affine_arg_index and get_shape(inputs[affine_arg_index]) is not None
...
# 修改操作名映射：PyTorch的aten操作名映射到Jittor
_JITTOR_OP_MAP = {
    "aten::addmm": "addmm",
    ...
    "aten::sub": "sub",
    "aten::div": "div",
    "aten::floor_divide": "floor_divide",
    "aten::relu": "relu",
    "aten::relu_": "relu_",
...
def flop_count(
    ...
    # Jittor不支持并行模型包装器的直接获取
    if hasattr(model, 'module'):  # 如果是包装器
        model = model.module
    ...
    # Jittor没有内置的trace功能，需要手动计算
    # 这里我们通过模型的前向传播来手动计算FLOPs
    # 注意：这是一个简化版本，实际的FLOPs计算可能需要更复杂的方法
    
    # 对于Jittor，我们需要使用不同的方法来获取计算图
    # 这里我们使用一个简化的方法来计算主要操作的FLOPs
    total_flop_counter = Counter()
    
    # 遍历模型的所有层
    def count_layer_flops(layer, x):
        nonlocal total_flop_counter
        layer_type = layer.__class__.__name__
        
        if isinstance(layer, nn.Linear):
            # 线性层: (batch, in_features) -> (batch, out_features)
            in_features = layer.in_features
            out_features = layer.out_features
            if isinstance(x, (list, tuple)):
                batch_size = x[0].shape[0] if hasattr(x[0], 'shape') else 1
            else:
                batch_size = x.shape[0] if hasattr(x, 'shape') else 1
            flops = batch_size * in_features * out_features
            total_flop_counter["linear"] += flops
            ...
            H_out = (H + 2 * layer.padding[0] - kernel_h) // stride_h + 1
            W_out = (W + 2 * layer.padding[1] - kernel_w) // stride_w + 1
            
            flops = batch * out_channels * H_out * W_out * in_channels * kernel_h * kernel_w
            total_flop_counter["conv"] += flops
            
        elif isinstance(layer, nn.BatchNorm2d):
            # BatchNorm: 每个元素4次操作
            if isinstance(x, (list, tuple)):
                num_elements = np.prod(x[0].shape)
            else:
                num_elements = np.prod(x.shape)
            total_flop_counter["batchnorm"] += num_elements * 4
            ...
            if isinstance(x, (list, tuple)):
                ...
                num_elements = np.prod(x.shape)
            total_flop_counter["norm"] += num_elements * 5
            ...
            if isinstance(x, (list, tuple)):
                ...
                num_elements = np.prod(x.shape)
            total_flop_counter["relu"] += num_elements
    ...
    # 创建一个包装函数来统计FLOPs
    def forward_with_flop_count(*args, **kwargs):
        # 这里可以尝试手动遍历模型层，但这是一个复杂的过程
        # 对于实际使用，建议使用Jittor的profiler或专门的分析工具
        return original_forward(*args, **kwargs)
    
    # 临时替换forward方法
    model.forward = forward_with_flop_count
    ...
    # 这里跳过了对未支持操作的警告，因为Jittor的trace机制不同
...
def benchmark():
    ...
    # 使用Jittor的设备管理
    model.execute = jt.make_module(model.execute, model.parameters())
    ...
    for imgid, img in enumerate(tqdm.tqdm(images)):
        ...
        if isinstance(img, (list, tuple)):
            inputs = [jt.array(i) if not isinstance(i, jt.Var) else i for i in img]
        else:
            inputs = [jt.array(img) if not isinstance(img, jt.Var) else img]
